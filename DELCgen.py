...
class Lightcurve(object):
    ...
    def Periodogram(self):
        ...
        '''
        if self.fft is None:
            self.fft = np.fft.fft(self.flux)
    
        periodogram = ((2.0 * self.tbin) / (self.length * (self.mean ** 2))) * np.abs(np.real(self.fft)) ** 2
        freq = np.arange(1, int(self.length // 2) + 1).astype(float) / (self.length * self.tbin)
    ...
    def Fit_PDF(self, initial_params=[6,6, 0.3,7.4,0.8,0.2],
        ...
        self.bins = nbins
        hist = np.histogram(self.flux, bins=nbins, density=True)
