...
class Lightcurve(object):
    ...
    def Fit_PDF(self, initial_params=[6,6, 0.3,7.4,0.8,0.2],
        ...
        self.bins = nbins
        hist_vals, bin_edges = np.histogram(self.flux, bins=nbins, density=True)
        hist = (hist_vals, bin_edges)
    ...
    def Plot_PDF(self,bins=None,norm=True,force_scipy=False):
        ...
        plt.hist(self.flux,bins=self.bins,density=True)
        #kappa,theta,lnmu,lnsig,weight
        if self.pdfFit:
    ...
    def Plot_Stats(self,bins=None,norm=True,force_scipy=False):
        ...
        plt.subplot(3,1,2)
        if bins:
            plt.hist(self.flux,bins=bins,density=True)        
        else:
            if self.bins == None:
                self.bins = OptBins(self.flux)
            plt.hist(self.flux,bins=self.bins,density=True)        
...
def Simulate_TK_Lightcurve(PSDmodel,PSDparams,lightcurve=None,
    ...
    two = False  # Flag for data without error column

    if fileroute.endswith(('.fits', '.fit')):
        # FITS FILE
        if time_col and flux_col:
            try:
                with fits.open(fileroute) as fitsFile:
                    data = fitsFile[extention].data
                    time = data.field(time_col)
            ...
            print("*** LOAD FAILED *** time_col and flux_col must be defined for FITS files.")
            return
    ...
    success = False
        ...
        try:
            ...
            if not success:
                print("*** LOAD FAILED *** No valid data found.")
                return
            print(f"Read {len(time)} lines of data.")
        except IOError:
